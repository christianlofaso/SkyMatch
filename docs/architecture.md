...
, but runs **after** store+prune, is **incremental** (only `parsed_at IS NULL` rows), and BOTH
  ...
  `upsert_listing` NULLs `parsed_at` when the listing content changes so it reparses.
  ...
  - **Two follow on passes** in the worker keep the precompute consistent without rerunning Haiku:
    ...
    `parsed_at` is set on parse success alone, so they'd otherwise never reenter the parse queue); the
    `scratch/backfill_company*.py` tools resolve companies again for already parsed rows (URL based +
