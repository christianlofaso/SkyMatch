...
stored in [`frontend/src/lib/demoRun.ts`](frontend/src/lib/demoRun.ts), prescored and
preannotated, so it makes **zero backend calls** and costs nothing. No `.env.local` needed; auth
