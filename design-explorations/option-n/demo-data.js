/* demo-data.js — SkyMatch "Tracker + Alerts" (option-n).
   Merges the two retention prototypes into one data layer:
     • from option-l: an application STATUS (saved/applied/interviewing/closed) + DEADLINE per role,
       persisted to localStorage, driving the pipeline strip, per-row status tag, deadlines + Nudge.
     • from option-k: an isNew flag + searchable `tags` + a `matchedSearch` (drawer provenance) per role,
       an SMSeen store ("viewing a role clears its New dot"), and an SM_SEARCHES array of saved searches.
   A row can wear BOTH a "New" dot (unseen alert) AND a status tag (in your pipeline) at once.
   Honest mechanics only: status never auto-advances, deadlines are user-entered, New counts clear
   only on a genuine open, no countdowns / shame metrics. Class names the CSS depends on are preserved. */

const SM_ROLES = [
  {
    id: "ramp-swe", band: "strong", bandLabel: "Strong match", type: "startup", typeLabel: "Startup",
    logo: "R", alt: false, isNew: true, tags: ["swe", "summer", "fintech"], matchedSearch: "fintech",
  ...
  },
  {
    ...
    logo: "C", alt: true, tags: ["swe", "near", "summer"], matchedSearch: "near",
  ...
  },
  {
    ...
    logo: "R", alt: false, isNew: true, tags: ["swe", "near", "summer", "fintech"], matchedSearch: "near",
  ...
  },
  {
    ...
    logo: "S", alt: true, isNew: true, tags: ["swe", "summer", "fintech"], matchedSearch: "fintech",
  ...
  },
  {
    ...
    logo: "V", alt: true, tags: ["swe", "summer", "frontend"], matchedSearch: "all",
  ...
  },
  {
    ...
    logo: "M", alt: false, isNew: true, tags: ["swe", "bay", "summer"], matchedSearch: "swe-bay",
  ...
  },
  {
    ...
    logo: "J", alt: true, tags: ["swe", "near", "summer", "fintech"], matchedSearch: "near",
  ...
  },
  {
    ...
    logo: "G", alt: false, isNew: true, tags: ["swe", "bay", "summer"], matchedSearch: "swe-bay",
  ...
  },
  {
    ...
    logo: "A", alt: true, isNew: true, tags: ["swe", "bay", "summer"], matchedSearch: "swe-bay",
  ...
  },
  {
    ...
    logo: "O", alt: false, tags: ["swe", "summer"], matchedSearch: "all",
  ...
  },
  {
    ...
    logo: "N", alt: true, tags: ["swe", "bay", "summer", "frontend"], matchedSearch: "swe-bay",
  ...
  },
  {
    ...
    logo: "P", alt: true, isNew: true, tags: ["swe", "bay", "summer", "fintech"], matchedSearch: "fintech",
  ...
  },
  {
    ...
    logo: "N", alt: false, tags: ["swe", "bay", "summer"], matchedSearch: "swe-bay",
  ...
  },
  {
    ...
    logo: "M", alt: true, tags: ["swe", "bay", "summer"], matchedSearch: "swe-bay",
  ...
  },
  {
    ...
    logo: "A", alt: false, tags: ["swe", "bay", "summer"], matchedSearch: "swe-bay",
...
/* ---- "Load more" intake pool ----
   Fresh roles that have NOT entered the pipeline yet. "Load more" at the feed bottom appends the next
   batch into the bands, giving the student new candidates to triage into their saved searches + pipeline.
   A couple are flagged isNew so a fresh page still raises the active search's new-count. */
const SM_MORE_ROLES = [
  {
    ...
    facts: { stipend: "$9,600/mo", location: "San Francisco, CA · Hybrid", term: "Summer 2026", team: "Risk Intelligence", co: "8,000 people · Private", posted: "1d ago" },
    have: ["Python", "Applied ML", "SQL"], need: []
  },
  {
    id: "huntington", band: "strong", bandLabel: "Strong match", type: "near", typeLabel: "Near you",
    logo: "H", alt: false, isNew: true, tags: ["swe", "near", "summer", "fintech"], matchedSearch: "near",
  ...
  },
  {
    ...
    logo: "L", alt: true, tags: ["swe", "summer", "frontend"], matchedSearch: "all",
  ...
  },
  {
    ...
    logo: "I", alt: false, tags: ["swe", "summer"], matchedSearch: "all",
  ...
  },
  {
    ...
    logo: "D", alt: false, isNew: true, tags: ["swe", "bay", "summer"], matchedSearch: "swe-bay",
  ...
  },
  {
    ...
    logo: "S", alt: false, tags: ["swe", "bay", "summer"], matchedSearch: "swe-bay",
...
/* ---- application pipeline stages (the tracker's spine — the DESTINATION) ---- */
const SM_STAGES = [
...
/* ---- saved searches (the INTAKE — what's arriving) ----
   Each saved search is a standing query over the inventory. `tags` are ALL-match required tags; the
   chip's new-count = roles that are isNew, still unseen, AND match this search. The "all" pseudo-search
   is the default (no filter). Counts are computed live in smNewCountFor() so opening a role updates them. */
const SM_SEARCHES = [
...
/* "today" for the demo, fixed so seeded deadlines + new-counts stay meaningful across reloads */
...
/* ---- save + status store (localStorage) ----
   Merges option-l's pipeline store (`sm:pipe`) with the legacy saved id list (`sm:saved`, kept for the
   sidebar badge + Saved page parity). `pipe` maps id → { status, deadline?, appliedAt?, savedAt? }.
   Saving a role with no status seeds it at "saved"; setting a later status records appliedAt the first
   time it crosses into Applied/Interviewing. Nothing here auto-advances — the user drives every move. */
const SM = {
  KEY: "sm:saved",
  PKEY: "sm:pipe",
  _read() {
  ...
  /* seed a believable pipeline on first load: one interviewing, two applied (with near-term deadlines),
     three saved (one stale → triggers a Nudge). Mirrors the funnel the alerts feed into. */
  pipe() {
...
/* ---- "seen" store (localStorage) — viewing a role clears its New dot ----
   From option-k. Opening the drawer marks a role seen; the dot + accent rail clear on next render and the
   saved-search counts recompute. Honest mechanic: nothing decays on its own — only a genuine open clears it. */
const SMSeen = {
  KEY: "sm:seen",
  _read() {
    try { const v = JSON.parse(localStorage.getItem(this.KEY)); return Array.isArray(v) ? v : null; }
...
/* every role the store can resolve = curated feed + intake pool (drawer/lookup needs both). */
...
/* live new-count for a saved search across a given pool of roles (defaults to the loaded feed). */
function smNewCountFor(searchId, pool) {
  return (pool || ALL_ROLES).filter(r => SMSeen.isFresh(r) && smRoleMatchesSearch(r, searchId)).length;
}

/* the saved search that surfaced a role, for the drawer provenance line. Falls back to a tag match
   (so an appended/edge role still names a believable search), then to "All matches". */
function smProvenanceFor(r) {
  let s = SM_SEARCHES.find(x => x.id === r.matchedSearch);
  if (!s || s.id === "all") {
    s = SM_SEARCHES.find(x => x.id !== "all" && smRoleMatchesSearch(r, x.id)) || SM_SEARCHES[0];
  }
  return s;
}
...
/* ---- company logos ----
   ...
   brand icons. Any load failure falls back to the original letter avatar. smLogoSrc is the one swap point. */
const SM_LOGO_DOMAINS = {
  ...
  "ibm": "ibm.com", "databricks": "databricks.com", "snowflake": "snowflake.com"
};
...
/* small clock/flag glyph for the deadline field label + provenance bell */
const SM_DEADLINE_ICO = _fIco('<path d="M4 22V4h12l-2 4 2 4H6"/>');
const SM_BELL_ICO = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.7 21a2 2 0 0 1-3.4 0"/></svg>';

/* a role's pipeline tag for the row: e.g. "Applied · Jun 14", "Interviewing", "Saved 9d ago" */
...
/* a soft, calm "Nudge" pill for stale saved roles (honest, no shame metric) */
function smNudgeHTML(r) {
  ...
  return `<span class="nudge" data-nudge="${r.id}" title="Saved ${SM.staleAge(r.id)} days ago — still open?">
    ${SM_BELL_ICO} Nudge</span>`;
}

/* the merge in one row: a "New" dot (unseen alert) on the title, AND any status/nudge tags below it. */
function smRowHTML(r) {
  ...
  return `
  <article class="role${fresh ? " is-new" : ""}${tracked ? " is-tracked" : ""}" data-band="${r.band}" data-type="${r.type}" data-stage="${SM.status(r.id) || ""}" data-tags="${(r.tags || []).join(" ")}" id="${r.id}">
    <button class="role-head" data-open="${r.id}" aria-haspopup="dialog">
      ${smLogoHTML(r)}
      <div class="meta">
        <div class="ti">${r.title}${newDot}</div>
...
/* ---- drawer (built once, filled per role) — the MERGE POINT ----
   Why-you-fit + Skill alignment (both bases) AND the option-l status stepper + deadline AND the option-k
   provenance line ("Matched: Risk & Fintech"). Opening a role clears its New dot. */
const SMDrawer = {
  el: null, scrim: null,
  init() {
    if (this.el) return;
  ...
  open(id) {
    ...
    if (!r) return;
    this.init();
    // viewing a role clears its "New" dot — mark seen, then let the host page re-render the rail/feed
    ...
    const prov = smProvenanceFor(r);
    // honest provenance copy: a narrow saved search reads "Matched: <name>"; a role that fits no
    // saved alert (only the broad run) says so plainly rather than implying an alert caught it.
    const provHTML = prov.id === "all"
      ? `${SM_BELL_ICO} Surfaced by your match run`
      : `${SM_BELL_ICO} Matched: <b>${prov.name}</b>`;
    ...
    this.el.innerHTML = `
      <div class="dr-head">
        ...
        <div class="dr-prov" title="The saved search that surfaced this role">${provHTML}</div>
      </div>
      <div class="dr-body">
