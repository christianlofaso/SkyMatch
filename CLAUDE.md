...
│   ├── worker/
│   │   └── ingest.py               # Standalone ingestion worker (python -m worker.ingest) → listing_store; no Sonnet/Opus, but its parse pass makes cheap Haiku + Voyage calls (both keys optional). Spend is tagged 'worker:ingest' (excluded from the user kill switch) + bounded by WORKER_SPEND_CAP_USD_DAILY
│   ├── config/
