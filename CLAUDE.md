...
│   │   └── cost.py                 # Token/cost accounting — cost_session/record_usage → sorted [cost] breakdown (est. USD per call)
...
│       └── internships.py       # POST /internships/search (zero-LLM serve) + /internships/annotate (deferred fit text); local live-fetch fallback
└── frontend/src/
...
| POST | `/internships/annotate` | `AnnotateRequest` (`profile`, `jobs: [{url, bucket}]`) | `application/x-ndjson` — one `AnnotateEnvelope` per line (`fit_explanation`/`reach_gap`), completion order; deferred "why you fit" for the feed |
...
- **`[cost]` logging mirrors `[timing]`** — `cost_session`/`record_usage` (`lib/cost.py`) wrap `/analyze` + `/analyze/batch` and print est-USD per Claude call + a sorted breakdown. Token counts are exact; **USD is approximate** (price table in `cost.py` — update it if pricing changes). Watch `cache_r` to confirm batch profile-caching is engaging.
