# Pathfinder Backend Deployment: Brainstorm / Discovery Notes
...
- **SQLite is a local file** — `DB_PATH = Path(__file__).parent / "pathfinder_cache.db"`, WAL mode. Needs a **persistent disk** that survives deploys/restarts, and the web process + worker must share the **same filesystem**.
